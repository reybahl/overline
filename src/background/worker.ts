...
const DOM_ELEMENT_RULES = [
  ...
  "- For copy/clipboard intents, use controlKind copy-button or elements whose text/ariaLabel mentions copy — never substitute Open/Download/Setup buttons",
...
const RECORD_AGENT_RULES = [
  ...
  "- If the intent mentions copy/clipboard, only click copy-button controls (or ariaLabel/text containing copy) — not Desktop/Download/Setup buttons",
...
function buildCompileScriptPrompt(
  ...
  return [
    ...
    "- For copy/clipboard intents: click the copy-button or clipboard-copy control — NOT the readonly input showing the URL/command",
    "- waitFor before a copy click must target the copy button (controlKind copy-button), not the input field",
    "- id values must be bare element ids (clone-with-gh-cli) — never prefix with #",
    ...
    "  · tag may be clipboard-copy for GitHub-style copy buttons",
