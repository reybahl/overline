...
const RECORD_AGENT_RULES = [
  ...
  "- Prefer 1–2 searchElements calls per turn. If the first query misses, try one broader alternate — then stop searching",
  "- Do not burn the tool budget on synonym variants (refresh/reload/Refresh/files refresh) of the same missing label",
  "- If searchElements returns nothing useful, try listElements once or waitFor — do not keep rephrasing the same query",
  "- Only use a search/list result when its text or ariaLabel clearly matches the intent; ignore unrelated hits",
  ...
  "- If a required control is missing from the page (not in search results with a matching label), use waitFor on a returned selector that clearly matches — do not substitute unrelated controls",
  "- Take the fewest steps — do not click jump/anchor links to scroll to a control that is already in the DOM",
  "- After the click that fully satisfies the intent, set done: true immediately — do not keep searching or adding steps",
...
function buildCompileScriptPrompt(
  ...
  return [
    ...
    "- Keep click when: no hrefSuffix, toggle/pressed state, in-page # fragment, button without href, same-page/reload href (hrefSuffix equals pageUrl path), or intent needs ordinal/relative targeting (first, latest, top)",
    ...
    "- Same-page/reload hrefSuffix (equals this step's pageUrl pathname) → omit all href fields; match ariaLabel/text/tag only",
    "- NEVER put {{segmentN}} in click/fill match fields (hrefSuffix, hrefContains, hrefPattern) — match strings are literal. {{segmentN}} is ONLY for navigate href",
    "",
    ...
    "- Same-page hrefSuffix (equals pageUrl path) → never navigate; keep click without href fields",
