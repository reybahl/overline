...
export function getRestrictedPageMessage(url: string | undefined): string {
  ...
  if (url.startsWith("chrome://") || url.startsWith("chrome-extension://")) {
    return "Overline can't run on Chrome system pages. Open a regular website first.";
  }

  if (url.startsWith("edge://") || url.startsWith("about:")) {
    return "Overline can't run on this browser page. Open a regular website first.";
  }

  if (url.startsWith("file://")) {
    return "Overline can't run on local files unless file access is enabled.";
  }

  return "Overline only works on http:// and https:// pages.";
}
